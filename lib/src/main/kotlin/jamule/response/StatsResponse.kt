...
data class StatsResponse(
    ...
    companion object {
        ...
        internal fun fromPacket(packet: Packet) = StatsResponse(
            ...
            uploadOverhead = packet.numeric(EC_TAG_STATS_UP_OVERHEAD)?.getNumber() ?: 0,
            downloadOverhead = packet.numeric(EC_TAG_STATS_DOWN_OVERHEAD)?.getNumber() ?: 0,
            bannedCount = packet.numeric(EC_TAG_STATS_BANNED_COUNT)?.getNumber() ?: 0,
            ...
            totalSentBytes = packet.numeric(EC_TAG_STATS_TOTAL_SENT_BYTES)?.getNumber() ?: 0,
            totalReceivedBytes = packet.numeric(EC_TAG_STATS_TOTAL_RECEIVED_BYTES)?.getNumber() ?: 0,
            sharedFileCount = packet.numeric(EC_TAG_STATS_SHARED_FILE_COUNT)?.getNumber() ?: 0,
            uploadSpeed = packet.numeric(EC_TAG_STATS_UL_SPEED)?.getNumber() ?: 0,
            downloadSpeed = packet.numeric(EC_TAG_STATS_DL_SPEED)?.getNumber() ?: 0,
            uploadSpeedLimit = packet.numeric(EC_TAG_STATS_UL_SPEED_LIMIT)?.getNumber() ?: 0,
            downloadSpeedLimit = packet.numeric(EC_TAG_STATS_DL_SPEED_LIMIT)?.getNumber() ?: 0,
            uploadQueueLength = packet.numeric(EC_TAG_STATS_UL_QUEUE_LEN)?.getNumber() ?: 0,
            totalSourceCount = packet.numeric(EC_TAG_STATS_TOTAL_SRC_COUNT)?.getNumber() ?: 0,
            ed2kUsers = packet.numeric(EC_TAG_STATS_ED2K_USERS)?.getNumber() ?: 0,
            kadUsers = packet.numeric(EC_TAG_STATS_KAD_USERS)?.getNumber() ?: 0,
            ed2kFiles = packet.numeric(EC_TAG_STATS_ED2K_FILES)?.getNumber() ?: 0,
            kadFiles = packet.numeric(EC_TAG_STATS_KAD_FILES)?.getNumber() ?: 0,
            kadNodes = packet.numeric(EC_TAG_STATS_KAD_NODES)?.getNumber() ?: 0,
            // Following metrics are only available if the client is connected to the Kad network.
            kadFirewalledUdp = (packet.byte(EC_TAG_STATS_KAD_FIREWALLED_UDP)?.getValue()?.let { !it.equals(0u) }),
            kadIndexedSources = packet.numeric(EC_TAG_STATS_KAD_INDEXED_SOURCES)?.getNumber(),
            kadIndexedKeywords = packet.numeric(EC_TAG_STATS_KAD_INDEXED_KEYWORDS)?.getNumber(),
            kadIndexedNotes = packet.numeric(EC_TAG_STATS_KAD_INDEXED_NOTES)?.getNumber(),
            kadIndexedLoad = packet.numeric(EC_TAG_STATS_KAD_INDEXED_LOAD)?.getNumber(),
        ...
        companion object {
            fun fromConStateTag(tag: Tag<*>): ConnectionState {
                ...
                return ConnectionState(
                    ...
                    ed2kId = tag.subtags.numeric(EC_TAG_ED2K_ID)?.getNumber(),
                    kadId = tag.subtags.numeric(EC_TAG_KAD_ID)?.getNumber(),
                    clientId = tag.subtags.numeric(EC_TAG_CLIENT_ID)?.getNumber(),
                    serverIpv4 = serverTag?.ipv4()?.getValue(),
                    serverPing = serverTag?.subtags?.numeric(EC_TAG_SERVER_PING)?.getNumber(),
                    serverPrio = serverTag?.subtags?.numeric(EC_TAG_SERVER_PRIO)?.getNumber(),
                    serverFailed = serverTag?.subtags?.numeric(EC_TAG_SERVER_FAILED)?.getNumber(),
                    serverStatic = serverTag?.subtags?.numeric(EC_TAG_SERVER_STATIC)?.getNumber()?.let { it != 0 },
                    ...
                    serverUsers = serverTag?.subtags?.numeric(EC_TAG_SERVER_USERS)?.getNumber(),
                    serverUsersMax = serverTag?.subtags?.numeric(EC_TAG_SERVER_USERS_MAX)?.getNumber(),
                    serverFiles = serverTag?.subtags?.numeric(EC_TAG_SERVER_FILES)?.getNumber(),
                )
            }
        }
    }
